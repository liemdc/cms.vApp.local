...
SET @stringId = (SELECT StringID FROM [CMS_ResourceString] LEFT JOIN [CMS_ResourceTranslation] ON StringID=TranslationStringID WHERE StringKey = N'settingscategory.cmsversioninguseobjectversioningfor' AND TranslationUICultureID = @cultureId);
IF  @stringId IS NULL
BEGIN
-- Insert resource string
INSERT INTO [CMS_ResourceString]
           ...
           ,[StringLoadGeneration])
     VALUES
           ('settingscategory.cmsversioninguseobjectversioningfor'
           ,0
           ,0)
...
/************** CHANGE GOOGLE ANALYTICS WEB PART PROPERTIES ***************/

UPDATE [CMS_WebPart]
SET [WebPartProperties] = '<form><category name="Google Analytics Settings" /><field column="TrackingCode" fieldcaption="Tracking code" visible="true" columntype="text" fieldtype="CustomUserControl" allowempty="false" isPK="false" system="false" columnsize="20" fielddescription="This code is obtained from the Google Analytics service at www.google.com/analytics." publicfield="false" spellcheck="false" guid="47bb33c6-3432-4fee-8dd2-204d2552ad5d" visibility="none" ismacro="false" hasdependingfields="false" dependsonanotherfield="false"><settings><controlname>textboxcontrol</controlname></settings></field><field column="TrackingType" fieldcaption="Tracking type" visible="true" defaultvalue="0" columntype="integer" fieldtype="CustomUserControl" allowempty="true" isPK="false" system="false" fielddescription="Type of the tracking depending on the domain model." publicfield="false" spellcheck="false" guid="0b69524d-3de0-440d-9c07-ec39ab5ecae5" visibility="none" ismacro="false" hasdependingfields="false" dependsonanotherfield="false"><settings><controlname>radiobuttonscontrol</controlname><RepeatDirection>vertical</RepeatDirection><Options>0;Single domain (e.g. only www.example.com)
1;One domain with multiple subdomains (e.g. sub1.example.com, sub2.example.com, sub3.example.com)
2;Multiple top-level domains (e.g. www.example.com, www.example.net, www.example.org)</Options></settings></field><field column="MainWebsiteDomain" fieldcaption="Website main domain" visible="true" columntype="text" fieldtype="CustomUserControl" allowempty="true" isPK="false" system="false" columnsize="100" fielddescription="Use this property to specify a custom domain name used for tracking purposes in non-trivial scenarios. For example, if you want to track visits to &quot;my.first.example.com&quot; and &quot;my.second.example.com&quot;, which  are both subdomains of &quot;example.com&quot;, fill in &quot;example.com&quot;. If you leave the property empty, the web part will use the current domain." publicfield="false" spellcheck="true" guid="7036b955-1f27-4e1a-abbb-8575661f9dfd" visibility="none" ismacro="false" hasdependingfields="false" dependsonanotherfield="false"><settings><Trim>False</Trim><AutoCompleteFirstRowSelected>False</AutoCompleteFirstRowSelected><AutoCompleteShowOnlyCurrentWordInCompletionListItem>False</AutoCompleteShowOnlyCurrentWordInCompletionListItem><FilterMode>False</FilterMode><AutoCompleteEnableCaching>False</AutoCompleteEnableCaching><controlname>textboxcontrol</controlname></settings></field><field column="UseAsyncScript" fieldcaption="Use asynchronous script" visible="true" defaultvalue="false" columntype="boolean" fieldtype="CustomUserControl" allowempty="true" isPK="false" system="false" fielddescription="Indicates if asynchronous variant of Google analytics script should be used." publicfield="false" spellcheck="false" guid="3f158398-bbf7-4061-8131-5777c6b2b04f" visibility="none" ismacro="false" hasdependingfields="false" dependsonanotherfield="false"><settings><controlname>checkboxcontrol</controlname></settings></field></form>'
WHERE [WebPartName]= N'GoogleAnalytics'
GO

/************ END CHANGE GOOGLE ANALYTICS WEB PART PROPERTIES *************/
...
-- Update View_OM_Account_MembershipJoined
IF EXISTS (SELECT 1 FROM [INFORMATION_SCHEMA].[VIEWS] WHERE [TABLE_NAME] = 'View_OM_Account_MembershipJoined')
DROP VIEW [View_OM_Account_MembershipJoined]
GO
...
-- Update View_OM_Account_Joined
IF EXISTS (SELECT 1 FROM [INFORMATION_SCHEMA].[VIEWS] WHERE [TABLE_NAME] = 'View_OM_Account_Joined')
DROP VIEW [View_OM_Account_Joined]
GO
...
SELECT     OM_Account.AccountID, OM_Account.AccountName, OM_Account.AccountAddress1, OM_Account.AccountAddress2, OM_Account.AccountCity, 
                      ...
                      ISNULL(PrimaryContact.ContactFirstName, '') + CASE WHEN (NULLIF(PrimaryContact.ContactFirstName,'') IS NULL) THEN '' ELSE ' ' END +
					  ISNULL(PrimaryContact.ContactMiddleName, '') + CASE WHEN (NULLIF(PrimaryContact.ContactMiddleName,'') IS NULL) THEN '' ELSE ' ' END +
					  ISNULL(PrimaryContact.ContactLastName, '') AS PrimaryContactFullName, ISNULL(SecondaryContact.ContactFirstName, '') + CASE WHEN (NULLIF(SecondaryContact.ContactFirstName,'') IS NULL) THEN '' ELSE ' ' END +
					  ISNULL(SecondaryContact.ContactMiddleName, '') + CASE WHEN (NULLIF(SecondaryContact.ContactMiddleName,'') IS NULL) THEN '' ELSE ' ' END +
					  ISNULL(SecondaryContact.ContactLastName, '') AS SecondaryContactFullName, ISNULL(OM_Account.AccountAddress1, '') + CASE WHEN (NULLIF(OM_Account.AccountAddress1,'') IS NULL) THEN '' ELSE ' ' END + OM_Account.AccountAddress2 AS AccountFullAddress
...
-- Update View_Poll_AnswerCount
IF EXISTS (SELECT 1 FROM [INFORMATION_SCHEMA].[VIEWS] WHERE [TABLE_NAME] = 'View_Poll_AnswerCount')
DROP VIEW [View_Poll_AnswerCount]
GO
...
SELECT	PollID, PollCodeName, PollDisplayName, PollTitle, PollOpenFrom, PollOpenTo, PollAllowMultipleAnswers, PollQuestion, PollAccess, PollResponseMessage, 
		PollGUID, PollLastModified, PollGroupID, PollSiteID, PollLogActivity, (SELECT SUM(AnswerCount) AS AnswerCount FROM Polls_PollAnswer WHERE (AnswerPollID = Polls_Poll.PollID)) AS AnswerCount
FROM	Polls_Poll
GO
...
(SELECT IPID, IPActiveContactID, IPOriginalContactID, IPAddress, IPCreated, ContactSiteID,
       ContactMergedWithContactID, ContactID, ISNULL(OM_Contact.ContactFirstName, '''') + CASE WHEN (NULLIF(OM_Contact.ContactFirstName,'''') IS NULL) THEN '''' ELSE '' '' END +
ISNULL(OM_Contact.ContactMiddleName, '''') + CASE WHEN (NULLIF(OM_Contact.ContactMiddleName,'''') IS NULL) THEN '''' ELSE '' '' END +
ISNULL(OM_Contact.ContactLastName, '''') AS ContactFullName
...
CMS_User.FirstName, CMS_User.LastName, CMS_User.Email, CMS_User.UserName, 
OM_Contact.ContactSiteID,  ISNULL(OM_Contact.ContactFirstName, '''') + CASE WHEN (NULLIF(OM_Contact.ContactFirstName,'''') IS NULL) THEN '''' ELSE '' '' END +
ISNULL(OM_Contact.ContactMiddleName, '''') + CASE WHEN (NULLIF(OM_Contact.ContactMiddleName,'''') IS NULL) THEN '''' ELSE '' '' END +
ISNULL(OM_Contact.ContactLastName, '''') AS ContactFullNameJoined  
FROM OM_Membership  
INNER JOIN OM_Contact ON OM_Membership.OriginalContactID=OM_Contact.ContactID  
...
WHERE ContactID IN (
SELECT * FROM Func_OM_Contact_GetChildren(@ContactID,1)) AND MemberType=0  ) 
as tab WHERE ##WHERE## ORDER BY ##ORDERBY##'
...
SET QueryText = 
'SELECT ##TOPN## ##COLUMNS## FROM (
SELECT ContactID, MembershipID, ActiveContactID, OriginalContactID, CustomerFirstName,
CustomerLastName, CustomerEmail, CustomerCompany,
ContactMergedWithContactID, OM_Contact.ContactSiteID, ISNULL(OM_Contact.ContactFirstName, '''') + CASE WHEN (NULLIF(OM_Contact.ContactFirstName,'''') IS NULL) THEN '''' ELSE '' '' END +
ISNULL(OM_Contact.ContactMiddleName, '''') + CASE WHEN (NULLIF(OM_Contact.ContactMiddleName,'''') IS NULL) THEN '''' ELSE '' '' END +
ISNULL(OM_Contact.ContactLastName, '''') AS ContactFullNameJoined  
FROM OM_Membership  
INNER JOIN OM_Contact ON OM_Membership.OriginalContactID=OM_Contact.ContactID  
INNER JOIN COM_Customer ON OM_Membership.RelatedID=COM_Customer.CustomerID  
WHERE ContactID IN (SELECT * FROM Func_OM_Contact_GetChildren(@ContactID, 1)) AND MemberType=1) 
as tab  WHERE ##WHERE## ORDER BY ##ORDERBY##' 
...
WHERE ContactID IN (SELECT * FROM Func_OM_Contact_GetChildren(@ContactID,1)) AND MemberType=2) 
as tab WHERE ##WHERE## ORDER BY ##ORDERBY##'
...
WHERE ##WHERE## ORDER BY ##ORDERBY##'
WHERE QueryName = N'selectfromview' AND ClassID = (SELECT ClassID FROM CMS_Class WHERE ClassName=N'OM.Contact')
GO
...
WHERE ##WHERE## ORDER BY ##ORDERBY##'
WHERE QueryName = N'selectmembershiplist' AND ClassID = (SELECT ClassID FROM CMS_Class WHERE ClassName=N'OM.Contact')
GO

UPDATE CMS_Query
SET QueryText = 
'SELECT ##TOPN## ##COLUMNS## FROM (SELECT OM_Contact.ContactID, ContactStatusID, ScoreID, Expiration, Value, OM_Contact.ContactSiteID,
ISNULL(OM_Contact.ContactFirstName, '''') + CASE WHEN (NULLIF(OM_Contact.ContactFirstName,'''') IS NULL) THEN '''' ELSE '' '' END +
ISNULL(OM_Contact.ContactMiddleName, '''') + CASE WHEN (NULLIF(OM_Contact.ContactMiddleName,'''') IS NULL) THEN '''' ELSE '' '' END +
ISNULL(OM_Contact.ContactLastName, '''') AS ContactFullNameJoined
FROM OM_Contact
INNER JOIN OM_ScoreContactRule ON OM_Contact.ContactID = OM_ScoreContactRule.ContactID) as OM_SelectScoreContact WHERE ##WHERE## ORDER BY ##ORDERBY##'
WHERE QueryName = N'selectscorecontact' AND ClassID = (SELECT ClassID FROM CMS_Class WHERE ClassName=N'OM.Contact')
GO
...
WHERE QueryName = N'selectusers' AND ClassID = (SELECT ClassID FROM CMS_Class WHERE ClassName=N'OM.Membership')
GO
...
INNER JOIN OM_Contact ON OM_Membership.OriginalContactID=OM_Contact.ContactID
INNER JOIN Newsletter_Subscriber ON OM_Membership.RelatedID=Newsletter_Subscriber.SubscriberID
WHERE ActiveContactID=@ContactID AND MemberType=2) as tab WHERE ##WHERE## ORDER BY ##ORDERBY##'
WHERE QueryName = N'selectsubscribers' AND ClassID = (SELECT ClassID FROM CMS_Class WHERE ClassName=N'OM.Membership')
GO
...
WHERE ##WHERE## ORDER BY ##ORDERBY##'
WHERE QueryName = N'selectmergedusers' AND ClassID = (SELECT ClassID FROM CMS_Class WHERE ClassName=N'OM.Membership')
GO
...
WHERE ##WHERE## ORDER BY ##ORDERBY##'
WHERE QueryName = N'selectmergedsubscribers' AND ClassID = (SELECT ClassID FROM CMS_Class WHERE ClassName=N'OM.Membership')
GO
...
DECLARE @num INT; SET @num = (SELECT RN FROM @list WHERE ActivityID = @ActivityID);
SELECT ActivityID, RN, @num AS BASE_RN FROM @list WHERE RN IN (@num - 1, @num + 1)'
WHERE QueryName = N'selectpreviousnextmergedcontact' AND ClassID = (SELECT ClassID FROM CMS_Class WHERE ClassName=N'OM.Activity')
GO
...
WHERE QueryName = N'selectcontactactivitylist' AND ClassID = (SELECT ClassID FROM CMS_Class WHERE ClassName=N'OM.Activity')
GO
...
DECLARE @num INT; SET @num = (SELECT RN FROM @list WHERE ActivityID = @ActivityID);
SELECT ActivityID, RN, @num AS BASE_RN FROM @list WHERE RN IN (@num - 1, @num + 1)'
WHERE QueryName = N'selectpreviousnext' AND ClassID = (SELECT ClassID FROM CMS_Class WHERE ClassName=N'OM.Activity')
GO
...
DECLARE @num INT; SET @num = (SELECT RN FROM @list WHERE ActivityID = @ActivityID);
SELECT ActivityID, RN, @num AS BASE_RN FROM @list WHERE RN IN (@num - 1, @num + 1)'
WHERE QueryName = N'selectpreviousnextcontact' AND ClassID = (SELECT ClassID FROM CMS_Class WHERE ClassName=N'OM.Activity')
GO
...
WHERE QueryName = N'selectcontactactivitymergedlist' AND ClassID = (SELECT ClassID FROM CMS_Class WHERE ClassName=N'OM.Activity')
GO
...
WHERE ##WHERE## ##ORDERBY##'
WHERE QueryName = N'selectmergedactivitylist' AND ClassID = (SELECT ClassID FROM CMS_Class WHERE ClassName=N'OM.Activity')
GO
...
(SELECT IPID, IPActiveContactID, IPOriginalContactID, IPAddress, IPCreated, ContactSiteID, ContactMergedWithContactID,
       ContactID, ISNULL(OM_Contact.ContactFirstName, '''') + CASE WHEN (NULLIF(OM_Contact.ContactFirstName,'''') IS NULL) THEN '''' ELSE '' '' END +
ISNULL(OM_Contact.ContactMiddleName, '''') + CASE WHEN (NULLIF(OM_Contact.ContactMiddleName,'''') IS NULL) THEN '''' ELSE '' '' END +
ISNULL(OM_Contact.ContactLastName, '''') AS ContactFullName
FROM OM_IP LEFT JOIN OM_Contact ON OM_IP.IPOriginalContactID = OM_Contact.ContactID WHERE ContactID IN (SELECT * FROM Func_OM_Contact_GetChildren(@ContactID, 1))
) as tab WHERE ##WHERE## ORDER BY ##ORDERBY##'
WHERE QueryName = N'selectmergedips' AND ClassID = (SELECT ClassID FROM CMS_Class WHERE ClassName=N'OM.IP')
GO
...
(SELECT IPAddress, IPID, IPActiveContactID, IPOriginalContactID, IPCreated, ContactSiteID,
       ContactID, ContactMergedWithContactID, ISNULL(OM_Contact.ContactFirstName, '''') + CASE WHEN (NULLIF(OM_Contact.ContactFirstName,'''') IS NULL) THEN '''' ELSE '' '' END +
ISNULL(OM_Contact.ContactMiddleName, '''') + CASE WHEN (NULLIF(OM_Contact.ContactMiddleName,'''') IS NULL) THEN '''' ELSE '' '' END +
ISNULL(OM_Contact.ContactLastName, '''') AS ContactFullName
FROM OM_IP LEFT JOIN OM_Contact ON OM_IP.IPOriginalContactID = OM_Contact.ContactID WHERE IPActiveContactID=@ContactID
) as tab WHERE ##WHERE## ORDER BY ##ORDERBY##'
WHERE QueryName = N'selectips' AND ClassID = (SELECT ClassID FROM CMS_Class WHERE ClassName=N'OM.IP')
GO
...
WHERE Expiration IS NULL OR (DATEDIFF(d, getdate(), Expiration) >= 0)
...
WHERE QueryName = N'selectcontactscore' AND ClassID = (SELECT ClassID FROM CMS_Class WHERE ClassName=N'OM.Contact')
GO
...
-- Update Proc_CMS_DocumentTag_RemoveTags
ALTER PROCEDURE [Proc_CMS_DocumentTag_RemoveTags]
...
-- Update Proc_CMS_Email_FetchEmailToSend
ALTER PROCEDURE [Proc_CMS_Email_FetchEmailToSend]
	@FetchFailed bit,
	@FetchNew bit,
	@FirstEmailID int,
	@BatchSize int
...
BEGIN
	...
	IF @FetchFailed = 1 AND @FetchNew = 1
		...
		INSERT INTO @Emails
			SELECT EmailID FROM CMS_Email WHERE (EmailStatus = 1 AND EmailID > @FirstEmailID) ORDER BY EmailPriority DESC, EmailID;

	ELSE IF @FetchNew = 1
		/* Get only waiting */
		INSERT INTO @Emails
			SELECT EmailID FROM CMS_Email WHERE (EmailStatus = 1 AND EmailID > @FirstEmailID AND EmailLastSendResult IS NULL) ORDER BY EmailPriority DESC, EmailID;

	ELSE IF @FetchFailed = 1
		/* Get only failed */
		INSERT INTO @Emails
			SELECT EmailID FROM CMS_Email WHERE (EmailStatus = 1 AND EmailID > @FirstEmailID AND EmailLastSendResult IS NOT NULL) ORDER BY EmailPriority DESC, EmailID;
...
-- Update Proc_Polls_PollAnswer_MoveDown
ALTER PROCEDURE [Proc_Polls_PollAnswer_MoveDown]
    @PollID int,
    @AnswerID int
AS
BEGIN
...
-- Create DeleteAll query for CMS_Tag --
DECLARE @TagClassID int;
SELECT @TagClassID=ClassID FROM CMS_Class WHERE ClassName = 'cms.tag'
DELETE FROM CMS_Query WHERE ClassID=@TagClassID AND QueryName='deleteall'
INSERT INTO CMS_Query ([QueryName], [QueryTypeID], [QueryText], [QueryRequiresTransaction], [ClassID], [QueryIsLocked],
  ...
  VALUES ( 'deleteall', 0, 'DELETE FROM CMS_Tag WHERE ##WHERE##', 0, @TagClassID, 0, GETDATE(), NEWID(), 0, 0)
...
-- Update CMS.EventLog.deleteolderlogs
UPDATE [CMS_Query]
   SET [QueryText] = '
WHILE 1=1
...
BEGIN
  ...
  (
    SELECT ROW_NUMBER() OVER(ORDER BY ##ORDERBY##) rn, EventID FROM [CMS_EventLog] WHERE ##WHERE##
  )
  DELETE TOP (100) FROM ROWSTODELETE WITH(READPAST) WHERE (rn > @LogMaxSize);
  IF (@@ROWCOUNT = 0) BREAK;
END;'
 WHERE [QueryName] = 'deleteolderlogs' AND [ClassID] IN (SELECT [ClassID] FROM [CMS_Class] WHERE [ClassName] = 'CMS.EventLog')
...
BEGIN
CREATE NONCLUSTERED INDEX [IX_OM_Contact_ContactLastName] ON [OM_Contact] 
(
	[ContactLastName]
)
END
...
/********************* UPDATE COMMUNITY.GROUP.REMOVEDEPENDENCIES *********************/
GO
ALTER PROCEDURE [Proc_Community_Group_RemoveDependencies]
@ID int
AS
BEGIN
SET NOCOUNT ON;
...
END
GO
/********************* END - UPDATE COMMUNITY.GROUP.REMOVEDEPENDENCIES *********************/

/* SET ACTIVATION DATE FOR ALL USERS */
DECLARE @HOTFIXVERSION INT;
SET @HOTFIXVERSION =  (SELECT KeyValue FROM [CMS_SettingsKey] WHERE KeyName = N'CMSHotfixVersion')
IF @HOTFIXVERSION  < 31
BEGIN

UPDATE CMS_UserSettings SET UserActivationDate = (SELECT TOP 1 UserCreated FROM CMS_User WHERE UserEnabled = 1 AND CMS_User.UserID = CMS_UserSettings.UserSettingsUserID) WHERE UserActivationDate IS NULL

END
GO

/* Remove unwanted constraints from CMS_Role table */
DECLARE @HOTFIXVERSION INT;
SET @HOTFIXVERSION =  (SELECT KeyValue FROM [CMS_SettingsKey] WHERE KeyName = N'CMSHotfixVersion')
IF @HOTFIXVERSION  < 32
BEGIN
IF  EXISTS (SELECT * FROM sysobjects WHERE id = OBJECT_ID(N'[DF_CMS_Role_RoleGroupID]') AND type = 'D')
BEGIN
	ALTER TABLE [CMS_Role] DROP CONSTRAINT [DF_CMS_Role_RoleGroupID]
END

IF  EXISTS (SELECT * FROM sysobjects WHERE id = OBJECT_ID(N'[DF_CMS_Role_RoleIsGroupAdministrator]') AND type = 'D')
BEGIN
	ALTER TABLE [CMS_Role] DROP CONSTRAINT [DF_CMS_Role_RoleIsGroupAdministrator]
END
END
...
/* ----------------------------------------------------------------------------*/
UPDATE [CMS_SettingsKey] SET KeyValue = '58' WHERE KeyName = N'CMSHotfixVersion'
GO
