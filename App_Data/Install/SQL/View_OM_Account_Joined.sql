...
SELECT     OM_Account.AccountID, OM_Account.AccountName, OM_Account.AccountAddress1, OM_Account.AccountAddress2, OM_Account.AccountCity, 
					  ...
					  ISNULL(PrimaryContact.ContactLastName, '') AS PrimaryContactFullName, ISNULL(SecondaryContact.ContactFirstName, '') +
					  CASE WHEN (NULLIF(SecondaryContact.ContactFirstName,'') IS NULL) THEN '' ELSE ' ' END + ISNULL(SecondaryContact.ContactMiddleName, '') +
					  CASE WHEN (NULLIF(SecondaryContact.ContactMiddleName,'') IS NULL) THEN '' ELSE ' ' END + ISNULL(SecondaryContact.ContactLastName, '') AS SecondaryContactFullName,
					  ISNULL(OM_Account.AccountAddress1, '') + CASE WHEN (NULLIF(OM_Account.AccountAddress1,'') IS NULL) THEN '' ELSE ' ' END + OM_Account.AccountAddress2 AS AccountFullAddress
