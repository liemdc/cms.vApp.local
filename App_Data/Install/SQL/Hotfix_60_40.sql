...
OM_Contact.ContactSiteID,  ISNULL(OM_Contact.ContactFirstName,'''') 
+  CASE OM_Contact.ContactFirstName    WHEN '''' THEN ''''    WHEN NULL THEN ''''    ELSE '' ''  END  
+ ISNULL(OM_Contact.ContactMiddleName,'''') 
+  CASE OM_Contact.ContactMiddleName    WHEN '''' THEN ''''    WHEN NULL THEN ''''    ELSE '' ''  END  
+ ISNULL(OM_Contact.ContactLastName, '''') AS ContactFullNameJoined  
FROM OM_Membership  
INNER JOIN OM_Contact ON OM_Membership.OriginalContactID=OM_Contact.ContactID  
...
+  CASE OM_Contact.ContactFirstName    WHEN '''' THEN ''''    WHEN NULL THEN ''''    ELSE '' ''  END  
+ ISNULL(OM_Contact.ContactMiddleName,'''') +  CASE OM_Contact.ContactMiddleName    WHEN '''' THEN ''''    WHEN NULL THEN ''''    ELSE '' ''  END  
+ ISNULL(OM_Contact.ContactLastName, '''') AS ContactFullNameJoined  
FROM OM_Membership  
INNER JOIN OM_Contact ON OM_Membership.OriginalContactID=OM_Contact.ContactID  
INNER JOIN COM_Customer ON OM_Membership.RelatedID=COM_Customer.CustomerID  
...
ISNULL(OM_Contact.ContactFirstName,'''') +  CASE OM_Contact.ContactFirstName    WHEN '''' THEN ''''    WHEN NULL THEN ''''    ELSE '' ''  END  
+ ISNULL(OM_Contact.ContactMiddleName,'''') +  CASE OM_Contact.ContactMiddleName    WHEN '''' THEN ''''    WHEN NULL THEN ''''    ELSE '' ''  END  
+ ISNULL(OM_Contact.ContactLastName, '''') AS ContactFullNameJoined  
FROM OM_Membership  
INNER JOIN OM_Contact ON OM_Membership.OriginalContactID=OM_Contact.ContactID  
...
/* ----------------------------------------------------------------------------*/
UPDATE [CMS_SettingsKey] SET KeyValue = '40' WHERE KeyName = N'CMSHotfixVersion'
GO
