...
INSERT INTO [Reporting_ReportTable] ([TableID], [TableName], [TableDisplayName], [TableQuery], [TableQueryIsStoredProcedure], [TableReportID], [TableSettings], [TableGUID], [TableLastModified]) VALUES (444, N'table', N'table', N'SET @FromDate ={%DatabaseSchema%}.Func_Analytics_DateTrim(@FromDate,''year'');
 SET @ToDate ={%DatabaseSchema%}.Func_Analytics_EndDateTrim(@ToDate,''year'');  
  ...
  ORDER BY ''{%ColumnName|(default)Hits%}'' DESC', 0, 619, N'<customdata><pagesize>15</pagesize><skinid>ReportGridAnalytics</skinid><querynorecordtext>No data found</querynorecordtext><pagemode>1</pagemode><enablepaging>False</enablepaging><exportenabled>True</exportenabled></customdata>', '3d8055e1-b10b-436d-a462-f511a797836c', '20110927 11:10:20')
...
INSERT INTO [Reporting_ReportTable] ([TableID], [TableName], [TableDisplayName], [TableQuery], [TableQueryIsStoredProcedure], [TableReportID], [TableSettings], [TableGUID], [TableLastModified]) VALUES (510, N'table', N'table', N'SELECT CampaignDisplayName AS '' '',ISNULL(SUM (Views.Hits),0) AS ''{$analytics_codename.visits$}'', ISNULL(SUM(Conversions.Hits),0) AS ''{$conversion.conversion.list$}'',
 ...
 FROM Analytics_Campaign 
  LEFT JOIN Analytics_Statistics ON CampaignName = SUBSTRING(StatisticsCode, 16, LEN(StatisticsCode)) OR (StatisticsObjectName = CampaignName)
-- Visits
LEFT JOIN (SELECT SUM(HitsCount) AS Hits,HitsStatisticsID AS HitsStatisticsID  FROM Analytics_YearHits 
...
LEFT JOIN (SELECT SUM(HitsCount) AS Hits,SUM(HitsValue) AS Value,HitsStatisticsID AS HitsStatisticsID  FROM Analytics_YearHits 
...
INSERT INTO [Reporting_ReportTable] ([TableID], [TableName], [TableDisplayName], [TableQuery], [TableQueryIsStoredProcedure], [TableReportID], [TableSettings], [TableGUID], [TableLastModified]) VALUES (516, N'table', N'table', N'SELECT CampaignDisplayName AS '' '', ConversionDisplayName AS ''{$Conversion.name$}'',ISNULL(SUM (Views.Hits),0) AS ''{$analytics_codename.visits$}'',
  ISNULL(SUM(Conversions.Hits),0) AS ''{$conversion.conversion.list$}'',
    ISNULL(CAST (CAST (CAST (SUM(Conversions.Hits) AS DECIMAL (15,2)) / SUM (Views.Hits) * 100 AS DECIMAL(15,1)) AS NVARCHAR(20))+''%'',''0.0%'') AS ''{$abtesting.conversionrate$}'',
  ISNULL(SUM (Conversions.Value),0) AS ''{$conversions.value$}'',  
  ISNULL(ROUND (SUM (Conversions.Value)  / NULLIF (SUM(Views.Hits),0), 1),0) AS ''{$conversions.valuepervisit$}''    
 FROM Analytics_Campaign 
  LEFT JOIN Analytics_Statistics ON @CampaignName = SUBSTRING(StatisticsCode, 16, LEN(StatisticsCode)) 
  LEFT JOIN Analytics_Conversion ON ConversionName = StatisticsObjectName
-- Visits
LEFT JOIN (SELECT SUM(HitsCount) AS Hits  FROM Analytics_YearHits LEFT JOIN Analytics_Statistics ON HitsStatisticsID = StatisticsID
        WHERE StatisticsCode = ''campaign'' AND StatisticsSiteID = @CMSContextCurrentSiteID AND StatisticsObjectName = @CampaignName)         
        AS Views ON 1=1
        
-- Conversion count, conversion value
LEFT JOIN (SELECT SUM(HitsCount) AS Hits,SUM(HitsValue) AS Value,HitsStatisticsID AS HitsStatisticsID  FROM Analytics_YearHits 
   GROUP BY HitsStatisticsID) AS Conversions ON Conversions.HitsStatisticsID = Analytics_Statistics.StatisticsID AND StatisticsCode LIKE ''campconversion;%''
WHERE CampaignSiteID = @CMSContextCurrentSiteID AND CampaignName = @CampaignName 
GROUP BY CampaignDisplayName,ConversionDisplayName', 0, 715, N'<customdata><pagesize>15</pagesize><skinid>ReportGridAnalytics</skinid><querynorecordtext>No data found</querynorecordtext><pagemode>1</pagemode><enablepaging>False</enablepaging><exportenabled>True</exportenabled></customdata>', '9dab3cd8-c9d1-40f1-95fd-19ad62e0f654', '20110712 19:11:11')
...
INSERT INTO [Reporting_ReportTable] ([TableID], [TableName], [TableDisplayName], [TableQuery], [TableQueryIsStoredProcedure], [TableReportID], [TableSettings], [TableGUID], [TableLastModified]) VALUES (545, N'table', N'table', N'SET @FromDate ={%DatabaseSchema%}.Func_Analytics_DateTrim(@FromDate,''month'');
SET @ToDate ={%DatabaseSchema%}.Func_Analytics_EndDateTrim(@ToDate,''month'');
SELECT X.Name AS ''{$mvtcombination.name$}'',ISNULL (SUM(Y.Hits),0) AS ''{$om.selectedperiod$}'',ISNULL(SUM(X.Hits),0) AS ''{$om.total$}''  FROM 
(
SELECT 
...
INSERT INTO [Reporting_ReportTable] ([TableID], [TableName], [TableDisplayName], [TableQuery], [TableQueryIsStoredProcedure], [TableReportID], [TableSettings], [TableGUID], [TableLastModified]) VALUES (550, N'table', N'table', N'SET @FromDate ={%DatabaseSchema%}.Func_Analytics_DateTrim(@FromDate,''month'');
SET @ToDate ={%DatabaseSchema%}.Func_Analytics_EndDateTrim(@ToDate,''month'');
SELECT X.Name AS ''{$mvtcombination.name$}'',ISNULL (SUM(Y.Hits),0) AS ''{$om.selectedperiod$}'',ISNULL(SUM(X.Hits),0) AS ''{$om.total$}''  FROM 
(
SELECT 
