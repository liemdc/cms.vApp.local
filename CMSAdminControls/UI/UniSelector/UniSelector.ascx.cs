...
public partial class CMSAdminControls_UI_UniSelector_UniSelector : UniSelector, IPostBackEventHandler, ICallbackEventHandler
{
    ...
    protected override void LoadViewState(object savedState)
    {
        ...
        if (Request.Form.AllKeys.Contains(hdnIdentificator.UniqueID))
        {
    ...
    private void LoadObjects(bool forceReload)
    {
        if (Object != null)
        {
            ...
            url += "?SelectionMode=" + SelectionMode + "&hidElem=" + hiddenField.ClientID + "&params=" + Server.UrlEncode(ScriptHelper.GetString(Identificator, false)) + "&clientId=" + ClientID + "&localize=" + (LocalizeItems ? 1 : 0) + AdditionalUrlParameters;
...
{ 
    ...
    /// <summary>
    /// Registers script notifying page that change uccurred.
    /// </summary>
    private void RegisterChangedScript()
    {
        ScriptHelper.RegisterStartupScript(Page, typeof(string), "US_Changed", "US_Changed();", true);
    }
