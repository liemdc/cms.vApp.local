...
public partial class CMSAdminControls_Validation_CssValidator : DocumentValidator
{
    ...
    public string ValidatorURL
    {
        get
        {
            ...
            {
                mValidatorURL = DataHelper.GetNotEmpty(SettingsHelper.AppSettings["CMSValidationCSSValidatorURL"], DEFAULT_VALIDATOR_URL);
            }
    ...
    protected void Page_PreRender(object sender, EventArgs e)
    {
        ...
        {
            string query = string.Format("?datakey={0}&docid={1}", Server.UrlEncode(ResultKey), Node.DocumentID);
            lnkNewWindow.OnClientClick = "modalDialog('" + ResolveUrl("~/CMSModules/Content/CMSDesk/Validation/ValidationResults.aspx") + query + "&hash=" + QueryHelper.GetHash(query) + "', 'ViewValidationResult', 800, 600);return false;";
        }
    }
    ...
    private void SetupControls()
    {
        ...
        string query = string.Format("?url={0}", Server.UrlEncode(Url));
        lnkViewCode.OnClientClick = "modalDialog('" + ResolveUrl("~/CMSModules/Content/CMSDesk/Validation/ViewCode.aspx") + query + "&hash=" + QueryHelper.GetHash(query) + "', 'ViewHTMLCode', 800, 600);return false;";
