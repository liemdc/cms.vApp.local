...
<frameset border="0" rows="<%= TabsBreadHeadFrameHeight %>, *" id="rowsFrameset">
    <frame name="shippingOptionHeader" src="ShippingOption_Edit_Header.aspx?shippingOptionID=<%=QueryHelper.GetInteger("shippingOptionID", 0)%>&siteId=<%=SiteID%>"
        scrolling="no" frameborder="0" noresize="noresize" />
    <frame name="shippingOptionContent" src="ShippingOption_Edit_General.aspx?ShippingOptionID=<%=QueryHelper.GetInteger("ShippingOptionID", 0)%>&saved=<%=QueryHelper.GetInteger("saved", 0)%> "
