...
<frameset border="0" rows="<%= TabsBreadHeadFrameHeight %>, *" id="rowsFrameset">
    <frame name="relationshipNameMenu" src="RelationshipName_Header.aspx?relationshipnameid=<%=QueryHelper.GetInteger("relationshipnameid", 0)%> "
        scrolling="no" frameborder="0" noresize="noresize" />
    <frame name="relationshipNameContent" src="RelationshipName_General.aspx?relationshipnameid=<%=QueryHelper.GetInteger("relationshipnameid", 0)%>&saved=<%=QueryHelper.GetInteger("saved", 0)%>"
