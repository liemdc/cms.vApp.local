DFU = {
    ...
    initializeDesign: function(containerId) {
        var container = jQuery('#' + containerId), innerDiv = container.find('div.InnerDiv'), uploaderDiv = container.find('div.UploaderDiv'), divLoaded = true;
    ...
    toogleLoading: function(containerId, show) {
        var container = jQuery('#' + containerId), loadingDiv = container.find('.LoadingDiv'), innerLoadingDiv = container.find('.innerLoadingDiv'), innerDiv = container.find('.InnerDiv'), uploaderDiv = container.find('.UploaderDiv');
        if (show) {
    ...
    OnUploadProgressChanged: function(containerId, totalUploadSize, totalUploaded, timeLeft) {
        ...
        var container = jQuery('#' + containerId), progressSpan = container.find('*[id$=Progress]'), cancelSpan = container.find('.UploadCancel'), percent = parseInt(totalUploaded / totalUploadSize * 100, 10);
        if (container.width() > 50) {
            ...
            if (cancelSpan.length === 0) {
                ...
                cancelSpan.click(function() {
                    try {
                        jQuery('#' + containerId).find('object')[0].Content.MultiFileUpload.CancelUpload()
                    } catch (e) {
                    }
                });
            }
