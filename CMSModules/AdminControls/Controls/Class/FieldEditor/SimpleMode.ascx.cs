...
public partial class CMSModules_AdminControls_Controls_Class_FieldEditor_SimpleMode : CMSUserControl
{
    ...
    public void LoadFieldTypes(FormUserControlTypeEnum type)
    {
        ...
        // Get control types with user controls
        DataSet controlTypes = FormHelper.GetFieldControlTypesWithUserControls(null, controls, true, isPrimary, type);
        // Localize display names
        if (!DataHelper.DataSourceIsEmpty(controlTypes))
        {
            foreach (DataRow row in controlTypes.Tables[0].Rows)
            {
                row["UserControlDisplayName"] = ResHelper.LocalizeString(ValidationHelper.GetString(row["UserControlDisplayName"], string.Empty));
            }
            controlTypes.AcceptChanges();
        }
        
        drpFieldType.DataSource = controlTypes;
