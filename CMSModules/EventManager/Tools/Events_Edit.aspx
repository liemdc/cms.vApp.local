...
<frameset border="0" rows="<%= TabsBreadHeadFrameHeight %>, *" id="rowsFrameset">
    <frame name="eventsHeader" src="Events_Header.aspx?eventid=<%=QueryHelper.GetInteger("eventId", 0)%> "
        scrolling="no" frameborder="0" noresize="noresize" id="pollMenu" />
    <frame name="eventsContent" src="Events_Attendee_List.aspx?eventid=<%=QueryHelper.GetInteger("eventId", 0)%>&saved=<%=QueryHelper.GetInteger("saved", 0)%> "
