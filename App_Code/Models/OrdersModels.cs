...
public class OrdersModels {
            ...
            try { 
                ...
                foreach (var MoldsProcess in ListMoldsProcess) {
                    LINQData.db.PM_ProjectProcesses.InsertOnSubmit(new PM_ProjectProcess() {
                        ProcessProjectTaskID = newTask.ProjectTaskID, ProcessListId = MoldsProcess.ProcessListId, ProcessRequired = true, ProcessGangerBrowse = false, ProcessSMGangerBrowse = false, CreatedWhen = DateTime.Now, CreatedByUserId = CMSContext.CurrentUser.UserID, ModifiedWhen = DateTime.Now, ModifiedByUserId = CMSContext.CurrentUser.UserID
                    });
                }
                    ...
                    if (UpdateProcess) { 
                        ...
                        foreach (var MoldsProcess in ListMoldsProcess) {
                            if (LINQData.db.PM_ProjectProcesses.FirstOrDefault(fod => fod.ProcessProjectTaskID == ProjectTaskID && fod.ProcessListId == MoldsProcess.ProcessListId) == null)
                                LINQData.db.PM_ProjectProcesses.InsertOnSubmit(new PM_ProjectProcess() {
                                    ProcessProjectTaskID = ProjectTaskID, ProcessListId = MoldsProcess.ProcessListId, ProcessRequired = true, ProcessGangerBrowse = false, ProcessSMGangerBrowse = false, CreatedWhen = DateTime.Now, CreatedByUserId = CMSContext.CurrentUser.UserID, ModifiedWhen = DateTime.Now, ModifiedByUserId = CMSContext.CurrentUser.UserID
                                });
                        }
                    }
                }
    ...
    public static List<ProjectProcessObject> ProjectProcessListByProjectTaskID(int ProjectTaskID) {
        return LINQData.db.PM_ProjectProcesses.Where(w => w.ProcessProjectTaskID == ProjectTaskID)
                ...
                .SelectMany(sm1 => sm1.userm.DefaultIfEmpty(), (sm1, userm) => new ProjectProcessObject { 
                    ProcessId = sm1.mp1.sm.mp.ProcessId, ProcessListId = sm1.mp1.sm.mp.ProcessListId, ProcessListName = sm1.mp1.pl.ProcessListName, UserModified = userm.FullName
                }).ToList();
    }
    public static void ProjectProcessCreated(int ProjectTaskID, int ProcessListId) {
        LINQData.db.PM_ProjectProcesses.InsertOnSubmit(new PM_ProjectProcess() {
            ProcessProjectTaskID = ProjectTaskID, ProcessListId = ProcessListId, ProcessRequired = true, ProcessGangerBrowse = false, ProcessSMGangerBrowse = false, CreatedWhen = DateTime.Now, CreatedByUserId = CMSContext.CurrentUser.UserID, ModifiedWhen = DateTime.Now, ModifiedByUserId = CMSContext.CurrentUser.UserID
        });
        LINQData.db.SubmitChanges();
    }
    ...
    public static List<OrdersProcessDetailObject> OrdersProcessListDetail(int ProjectTaskID, int ProcessListId) {
        return LINQData.db.PM_ProjectProcessDetails.Where(w => w.DetailProjectTaskID == ProjectTaskID)
            ...
            .Select(s => new OrdersProcessDetailObject { 
                DetailId = s.ppd.DetailId, ProjectTaskID = s.ppd.DetailProjectTaskID, DetailMachineId = s.ppd.DetailMachineId, SubProcessID = s.psp.SubProcessId, DetailStartTimeM = s.ppd.DetailStartTimeM, DetailEndTimeM = s.ppd.DetailEndTimeM, DetailOwnerM = s.ppd.DetailOwnerM, DetailTotalTimeM = s.ppd.DetailTotalTimeM
            }).ToList();
    }
    public static void OrdersProcessDetailCreated(int ProjectTaskID, int SubProcessID, int DetailMachineId, DateTime DetailStartTimeM, DateTime DetailEndTimeM, decimal DetailTotalTimeM, int DetailOwnerM) {
        LINQData.db.PM_ProjectProcessDetails.InsertOnSubmit(new PM_ProjectProcessDetail() {
            DetailProjectTaskID = ProjectTaskID, DetailSubProcessId = SubProcessID, DetailMachineId = DetailMachineId, DetailStartTimeM = DetailStartTimeM, DetailEndTimeM = DetailEndTimeM, DetailTotalTimeM = DetailTotalTimeM, DetailOwnerM = DetailOwnerM, DetailOwnerP = DetailOwnerM
        });
