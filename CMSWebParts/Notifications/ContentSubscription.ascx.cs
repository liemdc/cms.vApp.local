...
public partial class CMSWebParts_Notifications_ContentSubscription : CMSAbstractWebPart
{
    ...
    protected void SetupControl()
    {
        ...
        else
        {
            ...
            if (actions.Length > 0)
            {
                ...
                this.subscriptionElem.EventData1 = (String.IsNullOrEmpty(this.Path) ? "/%" : CMSContext.ResolveCurrentPath(this.Path));
    ...
    private NotificationTemplateInfo GetTemplateInfo(string templateName)
    {
        if (!string.IsNullOrEmpty(templateName))
        {
            ...
            if (!(SiteName.Equals("#current#", StringComparison.InvariantCultureIgnoreCase) || (SiteName == "-")))
            {
                siteName = SiteName;
            }

            if (templateName.StartsWith(siteName + ".", StringComparison.InvariantCultureIgnoreCase))
            {
