...
public partial class CMSDesk_Header : CMSDeskPage
{
    ...
    private void CheckUserImpersonate()
    {
        ...
        if ((user.IsGlobalAdministrator) && !RequestHelper.IsWindowsAuthentication())
        {
            ...
            ucUsers.WhereCondition = "UserID IN (SELECT UserID FROM CMS_UserSite WHERE SiteID = " + CMSContext.CurrentSiteID + " AND (UserIsGlobalAdministrator = 0 OR UserSiteManagerDisabled != 0)) AND (UserName <> N'public')";
